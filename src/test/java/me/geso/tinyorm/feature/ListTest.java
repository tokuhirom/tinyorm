...
public class ListTest extends TestBase {
	...
	public void beefffff() throws SQLException {
		...
		connection.prepareStatement("CREATE TABLE foo (id INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY, csvInt TEXT NOT NULL, csvString TEXT NOT NULL)").executeUpdate();
	}
	...
	public void testFoo() throws SQLException {
		...
		Foo foo = this.orm.insert(Foo.class).value("csvInt", ints).value("csvString", strings).executeSelect();
	...
	public static class Foo extends BasicRow<Foo> {
		...
		public static Object INFLATE(String column, Object value) {
			if ("csvInt".equals(column)) {
				...
				return Arrays.stream(split).map(it -> Integer.parseInt(it)).collect(Collectors.toList());
			}
		...
		public static Object DEFLATE(String column, Object value) {
			if ("csvInt".equals(column)) {
				if (value instanceof List) {
					...
					String string = list.stream().map(it -> it.toString()).collect(Collectors.joining(","));
					return string;
				}
			}
			if ("csvString".equals(column)) {
